...
namespace FTGAMEStudio.InitialFramework.Traverse
{
    ...
    /// 自助遍历器，基于委托事件处理方法事件。
    /// </summary>
    public class BuffetTraverser<TValue, TEnumerable> : Traverser<TValue, TEnumerable>, IFlowTraversable<TValue, TEnumerable> where TEnumerable : IEnumerable<TValue>
    {
        ...
        public BuffetTraverser(Action<TValue> onTraverse, Func<TValue, bool> isCanonical = null, FlowControl nonFlowControl = FlowControl.Continue, Action<TEnumerable> afterTraverse = null, Action<TEnumerable> beforeTraverse = null, Action<TValue> onNonCanonical = null) : base(nonFlowControl)
        {
            this.onTraverse = onTraverse ?? throw new ArgumentNullException(nameof(onTraverse), "For this parameter, you must provide a non-null value. (isCanonical)");
